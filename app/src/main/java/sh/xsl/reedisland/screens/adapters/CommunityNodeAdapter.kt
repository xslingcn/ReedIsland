...
package sh.xsl.reedisland.screens.adapters
...
class CommunityNodeAdapter(
    ...
    inner class TimelineProvider : ForumProvider() {
        ...
        override fun convert(helper: BaseViewHolder, item: BaseNode) {
            ...
            val resourceId: Int =
                context.resources.getIdentifier("bi_1", "drawable", context.packageName)
            helper.setImageResource(R.id.forumIcon, resourceId)
        }
    ...
    class TimelineCommunityNode(timelines: List<Timeline>, expanded: Boolean?) : BaseExpandNode() {
        ...
        override val childNode: MutableList<BaseNode> =
            timelines.map { TimelineNode(it) }.toMutableList()
    }
